...
vtkStandardNewMacro(vtkSQTubeFilter);

// Construct object with radius 0.5, radius variation turned off, the number
// of sides set to 3, and radius factor of 10.
//----------------------------------------------------------------------------
vtkSQTubeFilter::vtkSQTubeFilter()
{
  this->Radius = 0.5;
...
//----------------------------------------------------------------------------
int vtkSQTubeFilter::FillOutputPortInformation(
      int /*port*/,
      vtkInformation *info)
{
...
//----------------------------------------------------------------------------
int vtkSQTubeFilter::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  vtkPointSet *input=dynamic_cast<vtkPointSet*>(inInfo->Get(vtkDataObject::DATA_OBJECT()));
  vtkPolyData *inputPd=dynamic_cast<vtkPolyData*>(input);
  vtkUnstructuredGrid *inputUg=dynamic_cast<vtkUnstructuredGrid*>(input);
  if (inputPd)
    {
    inLines=inputPd->GetLines();
    }
  else
  if (inputUg)
    {
    inLines=inputUg->GetCells();
    }
  else
    {
    vtkErrorMacro("Unsupported input data type " << input->GetClassName());
    return 1;
    }

  vtkPoints *inPts=input->GetPoints();
  if ((inLines==0) || (inPts==0))
    {
    // vtkErrorMacro("Invalid dataset, must have non-empty lines.");
    return 1;
    }
...
//----------------------------------------------------------------------------
int vtkSQTubeFilter::GeneratePoints(vtkIdType offset,
...
//----------------------------------------------------------------------------
void vtkSQTubeFilter::GenerateStrips(vtkIdType offset, vtkIdType npts,
...
//----------------------------------------------------------------------------
void vtkSQTubeFilter::GenerateTextureCoords(vtkIdType offset,
                                          vtkIdType npts, vtkIdType *pts,
                                          vtkPoints *inPts,
...
//----------------------------------------------------------------------------
vtkIdType vtkSQTubeFilter::ComputeOffset(vtkIdType offset, vtkIdType npts)
{
  if ( this->SidesShareVertices )
    {
...
//----------------------------------------------------------------------------
const char *vtkSQTubeFilter::GetVaryRadiusAsString(void)
{
...
//----------------------------------------------------------------------------
const char *vtkSQTubeFilter::GetGenerateTCoordsAsString(void)
{
...
//----------------------------------------------------------------------------
void vtkSQTubeFilter::PrintSelf(ostream& os, vtkIndent indent)
{
